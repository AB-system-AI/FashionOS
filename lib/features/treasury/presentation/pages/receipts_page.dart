...
class _ReceiptsPageState extends ConsumerState<ReceiptsPage> {
  List<ReceiptVoucher> _items = const [];
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    final page = await ref.read(receiptServiceProvider).list(user!.tenantId!);
    if (!mounted) return;
    setState(() {
      _items = page.items;
      _loading = false;
  ...
  Future<void> _createSample() async {
    final user = ref.read(authControllerProvider).user;
    if (user == null) return;
    await ref.read(receiptServiceProvider).create(
          user: user,
          input: const ReceiptInput(payerName: 'Customer B', amount: 150, accountId: 'cash-1'),
        );
    await _load();
  }
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      subtitle: Text('${v.payerName} · ${v.amount.toStringAsFixed(2)}'),
                    ),
                  );
                },
              ),
