...
class _BankManagementPageState extends ConsumerState<BankManagementPage> {
  ...
  Future<void> _load() async {
    ...
    final page = await ref.read(bankServiceProvider).listBanks(user!.tenantId!);
    if (!mounted) return;
    setState(() {
      _items = page.items;
      _loading = false;
  ...
  Future<void> _createSample() async {
    final user = ref.read(authControllerProvider).user;
    if (user == null) return;
    await ref.read(bankServiceProvider).createBank(user: user, name: 'Sample Bank');
    await _load();
  }
      ...
      body: _loading
          ...
          : RefreshIndicator(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, i) {
                  final bank = _items[i];
                  return Card(child: ListTile(title: Text(bank.name), subtitle: Text(bank.code ?? 'No code')));
                },
              ),
            ),
    );
